...
export default function App() {
  ...
  useEffect(() => {
    ...
    socket.on("receive-message", ({ from, fromName, msg }) => {
      if (from !== socketId) setChat(prev => [...prev, { from, fromName, msg }]);
    });
    ...
    socket.on("offer", ({ from, signal }) => {
      ...
      if (mediaStream) mediaStream.getTracks().forEach(track => peer.addTrack(track, mediaStream));
      ...
      <div style={{ display: "flex", flex: 1 }}>
        ...
        <div style={{ flex: 2, display: "flex", flexDirection: "column", padding: 12 }}>
          ...
          <div style={{ flex: 1, position: "relative" }}>
            {sharedVideoUrl
              ? <ReactPlayer url={sharedVideoUrl} ref={playerRef} controls width="100%" height="100%" playing
